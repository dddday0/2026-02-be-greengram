...
public class MyFileUtil {
    ...
    public void makeFolders(String path) {
        ...
        //D:/woojeong/download/greengram + Path 경로, 예를 들어 path에 ddd/abc
        // 결과 ://D:/woojeong/download/greengram/ddd/abc 경로를 가진 파일 객체가 만들어진다.
        File file = new File(fileUploadPath, path);

        //해당 경로의 디렉토리가 없다면 디렉토리를 생성한다.
        if( !file.exists() ) {  //해당 경로의 디렉토리가 없다면
            file.mkdirs(); //폴더를 만든다. mkdirs는 중간경로에 디렉토리가 없어도 전부 디렉토리를 만들어 준다.
        }
    }
